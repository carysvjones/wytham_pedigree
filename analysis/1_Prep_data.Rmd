...
# clean dataset - add dummy names if want?
...
ring_gtit <- ring_all %>% 
  dplyr::filter(bto_species_code == 'GRETI' & age == 1) 
...
length(unique(breed_ring_gtit$Pnum)) #18131
...
length(unique(breed_gtit_noNA$Mother)) #8387
length(unique(breed_gtit_noNA$Father)) #6889
length(unique(breed_gtit_noNA$Pnum)) #13723
summary(breed_gtit_noNA$April.lay.date) #-5 to 83


nrow(breed_gtit_noNA) #13723


# ADD AGE  ----------------------------------------------------------------

breed_gtit_noNA_age <- clean$get_age(breed_gtit_noNA)
