...
#input breeding attempt data 1960-2022
breed <- read.csv(file.path(dirs$data, 'LT_breeding_data_greti_bluti_1960_2022.csv'), na.strings=c("", "NA"))
nrow(breed) #41987

# clean dataset 
breed <- clean_breeding_data(breed)
nrow(breed) #38017
...
#just keep great tits and those age 1 
ring_gtit <- ring_all %>% 
...
breed_ring_gtit <- merge(breed_gtit, ring_gtit, by = 'Pnum', all = T) %>%
  #filter out those with no bto_ring - leave in for now  are important
  #if have mother with no offspring, she might have a mother too, so want to keep her
  #will be filtered out when fix pedigree 
...
#make column for LD with dummy values to say if have LD recorded or not - just so can run this function 
...
ring <- read.csv(file.path(dirs$data, 'ebmp_database_ringing_record_export_GT&BT_all.csv'), na.strings=c("", "NA"))


breed_gtit_noNA_age <- get_age(breed_gtit_noNA, ring)
