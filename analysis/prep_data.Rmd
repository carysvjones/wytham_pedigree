...
breed_gtit_noNA_age <- get_age(ages, breed_gtit_noNA)
...
breed_gtit_noNA_age_sub <- remove_late_broods(breed_gtit_noNA_age, mothers = T, fathers = T, late = T)
...
gtit_data_sub_nb <- gtit_data_sub_nb %>%
  ...
  dplyr::mutate(x_mean = mean(x),
                ...
                area_polygon_crop3 = mean(area_polygon.G_crop3)) %>%
  dplyr::ungroup()
