...
LD_basic <- readRDS(file.path(dirs$output, 'LD_basic.rds'))
LD_NB <- readRDS(file.path(dirs$output, 'LD_NB.rds'))
LD_spat <- readRDS(file.path(dirs$output, 'LD_spat.rds'))
LD_envir <- readRDS(file.path(dirs$output, 'LD_envir.rds'))
...
#get subscript!!
ggplot(LD_basic_varcomp, aes(x = component, y = z.ratio)) + 
  geom_point() +
  xlab(expression('V'[BY]))
...
LD_varcomps <- rbind(get_var_comps(model = LD_basic), get_var_comps(model = LD_NB), 
                      get_var_comps(model = LD_spat), get_var_comps(model = LD_envir))
...
LD_herits <- tibble::tibble(name = c('Vp', 'Vp_yr', 'h2', 'h2_yr'),
                            Est = c(#Vp - total phenotypic var
                                    sum(subset(LD_varcomps, model_name == 'LD_basic')$Est, na.rm = T ),
                                    #Vp_yr, within year phenotypic var
                                    sum(subset(LD_varcomps, model_name == 'LD_basic' & name != 'Vby')$Est,
                                        na.rm = T),
                                    #h2 - across years
                                    subset(LD_varcomps, model_name == 'LD_basic' & name == 'Va')$Est / 
                                      sum(subset(LD_varcomps, model_name == 'LD_basic')$Est, na.rm = T),
                                    #within year h2
                                    subset(LD_varcomps, model_name == 'LD_basic' & name == 'Va')$Est / 
                                      sum(subset(LD_varcomps, model_name == 'LD_basic' & name != 'Vby')$Est,
                            ...
                            SE = c(#get SE for these - first Vp
                                   NA, NA,
                                   #h2 - across years
                                    subset(LD_varcomps, model_name == 'LD_basic' & name == 'Va')$SE / 
                                      sum(subset(LD_varcomps, model_name == 'LD_basic')$SE, na.rm = T),
                                   #within year h2
                                    subset(LD_varcomps, model_name == 'LD_basic' & name == 'Va')$SE / 
                                      sum(subset(LD_varcomps, model_name == 'LD_basic' & name != 'Vby')$SE)))
                                   
                                    

LD_herits <- LD_herits %<% 
  dplyr::mutate(Est = subset(LD_varcomps, model_name == 'LD_basic'))



#WHEN come to make table for paper - will need to rearrange preevious
#so not long, but wide 
#and use these columns
# #make column with estimate and standard error for pretty table - here or later?
#   dplyr::mutate(Est_SE = paste0(format(round(Est, 2), nsmall = 2), ' (', 
#                                format(round(SE, 2), nsmall = 2), ')''),
#                   #get column with proportion relative to VP and SE
#                 Rel_Est_SE = paste0(format(round(Est / sum(Est), 2), nsmall = 2), ' (', 
#                                    format(round(((SE / Est) * (Est / sum(Est))), 2),
#                                           nsmall = 2), ')'))
...
round(LD_basic_test$component, 2)
                               

LD_basic_varcomp <- tibble::tibble(name = list_components,
                                   Est_SE = name[1]) 

#make to little tables of raw var comps
LD_basic_varcomp <- summary(LD_basic)$varcomp
LD_basic_varcomp <- tibble::rownames_to_column(LD_basic_varcomp, var = "name")
LD_basic_varcomp$model <- 'Mod1'

LD_NB_varcomp <- summary(LD_NB)$varcomp
LD_NB_varcomp <- rownames_to_column(LD_NB_varcomp, var = "name")
LD_NB_varcomp$model <- 'Mod2'

LD_dist_varcomp <- summary(LD_dist)$varcomp
LD_dist_varcomp <- rownames_to_column(LD_dist_varcomp, var = "name")
LD_dist_varcomp$model <- 'Mod3'

LD_envir_varcomp <- summary(LD_envir)$varcomp
LD_envir_varcomp <- rownames_to_column(LD_envir_varcomp, var = "name")
LD_envir_varcomp$model <- 'Mod4'
...
LD_basic.VPtot <- asreml::vpredict(LD_basic, LD_basic.VPtot ~ V1 + V2 + V3 + V4)
LD_NB.VPtot <- asreml::vpredict(LD_NB, LD_NB.VPtot ~ V1 + V2 + V3 + V4 + V5)
LD_dist.VPtot <- asreml::vpredict(LD_dist, LD_dist.VPtot ~ V1 + V2 + V3 + V4 + V5)
LD_envir.VPtot <- asreml::vpredict(LD_envir, LD_envir.VPtot ~ V1 + V2 + V3 + V4 + V5)

#total phenotypic variance within year
LD_basic.VP <- asreml::vpredict(LD_basic, LD_basic.VP ~  V2 + V3 + V4)
LD_NB.VP <- asreml::vpredict(LD_NB, LD_NB.VP ~ V2 + V3 + V4 + V5)
LD_dist.VP <- asreml::vpredict(LD_dist, LD_dist_test.VP ~ V2 + V3 + V4 + V5)
LD_envir.VP <- asreml::vpredict(LD_envir, LD_envir.VP ~ V2 + V3 + V4 + V5)

#heritability - total
LD_basic.h2 <- asreml::vpredict(LD_basic, LD_basic.h2 ~ V2 / (V2 + V3 + V4))
LD_NB.h2 <- asreml::vpredict(LD_NB, LD_NB.h2 ~ V3 / (V2 + V3 + V4 + V5))
LD_dist.h2 <- asreml::vpredict(LD_dist, LD_dist.h2 ~ V3 / (V2 + V3 + V4 + V5))
...
LD_NB.h2tot <- asreml::vpredict(LD_NB, LD_NB.h2tot ~ V3 / (V1 + V2 + V3 + V4 + V5))
LD_dist.h2tot <- asreml::vpredict(LD_dist, LD_dist.h2tot ~ V3 / (V1 + V2 + V3 + V4 + V5))
LD_envir.h2tot <- asreml::vpredict(LD_envir, LD_envir.h2tot ~ V3 / (V1 + V2 + V3 + V4 + V5))


#prop explained by NB, and matrices within year
LD_NB.SPACEprop <- asreml::vpredict(LD_NB, LD_NB.SPACEprop ~ V2 / (V2 + V3 + V4 + V5))
LD_dist.SPACEprop <- asreml::vpredict(LD_dist, LD_dist.SPACEprop ~ V2 / (V2 + V3 + V4 + V5))
LD_envir.SPACEprop <- asreml::vpredict(LD_envir, LD_envir.SPACEprop ~ V2 / (V2 + V3 + V4 + V5))


#get prop explained by each var comp
#breed year
LD_basic.BY <- asreml::vpredict(LD_basic, BY ~ V1)
LD_NB.BY <- asreml::vpredict(LD_NB, BY ~ V1)
LD_dist.BY <- asreml::vpredict(LD_dist, BY ~ V1)
LD_envir.BY <- asreml::vpredict(LD_envir, BY ~ V1)
#mother vm
LD_basic.VM <- asreml::vpredict(LD_basic, VM ~ V2 )
LD_NB.VM <- asreml::vpredict(LD_NB, VM ~ V3 )
LD_dist.VM <- asreml::vpredict(LD_dist, VM ~ V3 )
LD_envir.VM <- asreml::vpredict(LD_envir, VM ~ V3 )
#mother ide
LD_basic.IDE <- asreml::vpredict(LD_basic, IDE ~ V3)
LD_NB.IDE <- asreml::vpredict(LD_NB, IDE ~ V4)
LD_dist.IDE <- asreml::vpredict(LD_dist, IDE ~ V4)
LD_envir.IDE <- asreml::vpredict(LD_envir, IDE ~ V4)
#residuls
LD_basic.R <- asreml::vpredict(LD_basic, R ~ V4)
LD_NB.R <- asreml::vpredict(LD_NB, R ~ V5)
LD_dist.R <- asreml::vpredict(LD_dist, R ~ V5)
LD_envir.R <- asreml::vpredict(LD_envir, R ~ V5)
#matrix
LD_basic.NONE <- 0
LD_NB.NB <- asreml::vpredict(LD_NB, NB ~ V2)
LD_dist.DIST <- asreml::vpredict(LD_dist, DIST ~ V2)
LD_envir.ENVIR <- asreml::vpredict(LD_envir, ENVIR ~ V2)
...
basic_comb <- tibble::rownames_to_column(basic_comb, var = "variable")
...
NB_comb <- tibble::rownames_to_column(NB_comb, var = "variable")
...
dist_comb <- tibble::rownames_to_column(dist_comb, var = "variable")
...
envir_comb <- tibble::rownames_to_column(envir_comb, var = "variable")
...
herit <- tibble::rownames_to_column(herit, var = "model")
