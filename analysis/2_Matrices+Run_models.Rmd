...
env_matrix <- readRDS(file.path(dirs$data, 'env_matrix.rds'))
...
subset(spatial_matrix, spatial_matrix[,4] == 16)

saveRDS(spatial_matrix, file = file.path(dirs$data,'spatial_matrix.rds'))

spatial_matrix <- readRDS(file.path(dirs$data, 'spatial_matrix.rds'))
...
#model basic 
LD_basic_test2 <- asreml::asreml(fixed = April.lay.date ~ 1 + Fbreeding_age_group,
...
#model with NB
LD_NB_2 <- asreml::asreml(fixed = April.lay.date ~ 1 + Fbreeding_age_group,
                random = ~vm(Mother, ped_data_inv) + 
                  ide(Mother, ped_data_inv) +
                  breeding_year + 
                  nest.box,
